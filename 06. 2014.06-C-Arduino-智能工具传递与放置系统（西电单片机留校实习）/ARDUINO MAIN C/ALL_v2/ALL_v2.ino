...
const int pulsesmax[]={1850,1000,800,700,900};//定义最大脉宽
...
void setup()
{
  ...
  lcd.backlight();
  LCDinit();
  //MsTimer2::set(18, flash); // 500ms period
  //MsTimer2::start();
}
...
void stmovcontrol(int angl1,int angl2,int angl3,int angl4,int angl5)//定义一个脉冲函数
{
  ...
  int pulsestemp[]={pulsesnow[0],pulsesnow[1],pulsesnow[2],pulsesnow[3],pulsesnow[4]};
  int temp,raw[]={0,1,2,3,4};
  for(int j=0;j<4;j++)
  {
      for(int i=0;i<4-j;i++)
      {
          if(pulsestemp[i]>pulsestemp[i+1])//数组元素大小按升序排列
          {
              temp=pulsestemp[i];
              pulsestemp[i]=pulsestemp[i+1];
              pulsestemp[i+1]=temp;
              temp=raw[i];
              raw[i]=raw[i+1];
              raw[i+1]=temp;
  ...
  for(int i=0;i<5;i++)
  {
    digitalWrite(servopin[raw[i]],HIGH);//将舵机接口电平至高
    //delayMicroseconds(pulsestemp[i+1]-pulsestemp[i]);
  }
  delayMicroseconds(pulsestemp[0]);
  digitalWrite(servopin[raw[0]],LOW);
  for(int i=1;i<5;i++)
  {
    delayMicroseconds(pulsestemp[i]-pulsestemp[i-1]);
    digitalWrite(servopin[raw[i]],LOW);//将舵机接口电平至low  
  }
