...
if __name__ == '__main__':
    ...
    freq, Signals_N = Gen.GetWavs2(
        'D:/库/文档/GitHub/fast_bss/Code/wavs/arctic_4s (', ').wav', range(1,8+1))
    m,n=Signals_N.shape
    
    fig = plt.figure(figsize=(5, 8))
    ax = plt.subplot(1,1,1)
    ax.set_title("S value density distribution")
    ax.hist(Signals_N[1,:], bins=1000, color='black', normed=False )#/max(Signals_N[0,:]
    
    
    Signals_N,V=whiten(Signals_N)
    print(V)
    ...
    #ax = plt.subplot(1,1,1)
    ax.set_title("S value density distribution")
    ax.hist(Signals_N[1,:], bins=1000, color='steelblue', normed=False )#/max(Signals_N[0,:])
    ...
    for i in range(n):
        #n=1
        _hist[np.int(round(_normal_sigs[i]*100))]+=1
    _hist=_hist/n
    #for i in range(100):
        #print(round(_hist[i],3))
