...
class DataService
{
  public:
    ...
    static void *UDPSvr(void *args) 
    {
        ...
        {
            TT1.start();
            switch (DI.Recv(_state))
            {
            case SdMod.Move_Return:
                pthread_mutex_lock(&recv_mutex);
                my->recv_state = _state;
                _time = my->send_time;
                //std::cout << " recv " <<ccount++<< std::endl;
                /*
                status = STOP;//STOP
                while(!status)
                {
                ...
                //std::cout << " sent " <<ccount++<< std::endl;
                pthread_mutex_unlock(&recv_mutex);
                DI.Send(SdMod.RTime,_time);
                break;
            case SdMod.Move:
                pthread_mutex_lock(&recv_mutex);
                my->recv_state = _state;
            ...
            case SdMod.Info: 
                ...
                _time=my->send_time;
                pthread_mutex_unlock(&recv_mutex);
                DI.Send(SdMod.RInfo,_state, _time);
                break;
            case SdMod.End:
                break;
            default:
