...
byte TimeSpan; //用来对角速度积分的
float count;


void FLOAT32_SEND(float float32)
{
  int int16;
  int16=int(min(float32*100,32767));
   Serial.write(byte(int16>>14)&0x3F);
   Serial.write(byte(int16>>7)&0x3F);
   Serial.write(byte(int16)&0x3F);  
}
 void DATA_SEND(float ax,float ay,float az,float gx,float gy,float vx,float vy,float vz,byte time)
 {
 Serial.write(254);
 FLOAT32_SEND(ax);
 FLOAT32_SEND(ay);
...
void loop() {
 delay(20);
  accelgyro.getMotion6(&ax, &ay, &az, &gx, &gy, &gz);
    aax=ax/1638.400-Ax_offset;
    aay=ay/1638.400-Ay_offset;
    aaz=az/1638.400-Az_offset;
...
//下面三行就是通过对角速度积分实现各个轴的角度测量，当然假设各轴的起始角度都是0
 ...
 DATA_SEND(Ax,Ay,Az,Gx,Gy,Vx,Vy,Vz,TimeSpan);
