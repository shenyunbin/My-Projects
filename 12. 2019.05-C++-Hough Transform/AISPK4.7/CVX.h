...
public:
	...
	void ConvolveFilter3_3(T1& image, T2& image_filt, 
	...
	{
		return ((abs(R - 255) > color_threshold) || (abs(G - 255) > color_threshold) || (abs(B-0) > color_threshold)) ? 0 : 1;
	}
	...
	HTParameter HoughTransform(T &EdgeMask, const int r_min, const int r_max, const int r_step,
		bool const ht_gaussfilter, const int kernel[3][3]=kernel_gauss, const int boundary_condition=-1)
	{
		HTParameter _HTP, _HTP_MAX;
		_HTP_MAX.max_accu_value = 1;
		for (int r = r_min; r < r_max; r += r_step)
		{
			_HTP = SubHoughTransform(EdgeMask, r, ht_gaussfilter, kernel, boundary_condition);
	...
	HTParameter SubHoughTransform(T &EdgeMask, 
		int radius, bool const ht_gaussfilter, const int kernel[3][3], const int boundary_condition)
	{
		...
		HTParameter _HTP = HTGetMaxAccu(TempData->AccuGrid, ht_gaussfilter, kernel, boundary_condition);
		return _HTP;
	}
	...
	HTParameter HTGetMaxAccu(T &AccuGrid, 
		bool const ht_gaussfilter, const int kernel[3][3], const int boundary_condition)
	{
		//filter enabled
		if (ht_gaussfilter) 
		{
			...
			return SubHTGetMaxAccu(TempData->AccuGrid_filt);
		}
		//filter disabled
		else
		{
			return SubHTGetMaxAccu(TempData->AccuGrid);
		}
	}

	//get the maximal value in the ht zone
	template<class T>
	HTParameter SubHTGetMaxAccu(T& AccuGrid)
	{
		//define ht parameter
		HTParameter _HTP = { 0 };
	...
	HTParameter HTFindBall(T &R, T &G, T &B,
		...
		bool const ht_gaussfilter=false,
		const int ht_gaussfilter_kernel[3][3]= kernel_gauss, const int ht_boundary_condition = -1)
	{
		...
		Result = HoughTransform(Data->EdgeMask, r_min, r_max, r_step, ht_gaussfilter,
			ht_gaussfilter_kernel, ht_boundary_condition);
		return Result;
	}
